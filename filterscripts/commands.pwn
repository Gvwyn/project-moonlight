...
CMD:pos(playerid, params[])
{
	...
	SendClientMessage(playerid, 0x33FFAAAA, "Your current position is %f, %f, %f; rotation: %.2f", x, y, z, angle);
	return 1;
}
...
CMD:sound(playerid, params[])
{
	...
	{
		SendClientMessage(playerid, 0x007AFFFF, "Playing sound. ID: %i", sound);
		PlayerPlaySound(playerid, sound, 0.0, 0.0, 0.0);
		return 1;
	}
	else
	{
		SendClientMessage(playerid, 0xFF0000FF, "/sound <ID>");
		return 1;
	}
}
...
CMD:v(playerid, params[])
{
	...
	if(!sscanf(params, "i", vehicleid))
	{
	    ...
	    {
	    	SendClientMessage(playerid, 0xFF0000AA, "Vehicle IDs range from 400 to 611. (%i)", vehicleid);
	    	return 1;
	    }
	}
	else
	{
		SendClientMessage(playerid, 0xFF0000AA, "/v <ID / name>");
		return 1;
	}
}
...
CMD:ghost(playerid, params[])
{
	if (GetPlayerGhostMode(playerid) == false)
	{
		...
		SendClientMessage(playerid, 0x00AA00FF, "Ghost mode enabled.");
		return 1;
	}
	TogglePlayerGhostMode(playerid, false);
	DisableRemoteVehicleCollisions(playerid, false);
	SendClientMessage(playerid, 0xAA0000FF, "Ghost mode disabled.");
	return 1;
}
...
CMD:stats(playerid, params[])
{
	...
	if (!sscanf(params, "i", id))
	{
		...
		{
			SendClientMessage(playerid, 0xFF0000FF, "Player ID %i is not online.", id);
			return 1;
		}
		else passingID = id;
	}
	...
	if (passingID == playerid || IsPlayerAdmin(playerid))
	{
		DB_GetFieldStringByName(Result, "Password", pass, 64);
	}
	else pass = "{FFFFFF}Hidden";
	...
	Dialog_Show(playerid, STATS, DIALOG_STYLE_MSGBOX, "{00FF00}Player Statistics",\
	"{FFFFFF}UID:\t\t{00FFFF}%s\n\
	{FFFFFF}Name:\t\t{%06x}%s\n\
